  ...
  <div class="review">
    ...
    <div class="reviewContainer" v-loading="fullscreenLoading" v-if="!is2c">
      ...
      <div class="reviewBottom">
        <el-button type="primary" round class="back" @click="back"
          >返回</el-button
        >
        <el-button type="primary" round class="confirm" @click="confirm"
          >确认提交</el-button
        >
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    postFileHashs() {
      ...
      this.$http
        .post(process.env.API_ROOT + "api/v1/attestations/put", {
        ...
        if (valid) {
          ...
          try {
            let res = await this.$http.post(
              process.env.TOC_API_ROOT + "api/v1/c/attestation/put",
              prs
            );
            ...
            params.params.qrcodeUrl =
              process.env.TOC_API_ROOT + params.params.qrcodeUrl;
            params.params.callback =
              process.env.TOC_API_ROOT + params.params.callback;
    ...
    async checkPay() {
      let res = await this.$http.post(
        process.env.TOC_API_ROOT + "api/v1/c/attestation/hash",
        {
          access_token: this.access_token,
          hash: this.checkHash
        }
